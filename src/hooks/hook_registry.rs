...
impl<T: PacketType + Send, U: PacketType + Send> Hook<T, U> {
    ...
    ///
    /// ```
    /// let my_hook = Hook::new("My first hook", Box::new(|services, packet| { println!(packet.id); }));
    /// ```
...
impl<T: PacketType + Send, U: PacketType + Send> HookRegistry<T, U> {
    ...
    pub async fn run_hooks(&self, packet: &mut packet_context::PacketContext<T, U>) -> Result<(), HookError> {
    ...
    fn _can_execute(&self, exec_code: &HashMap<Uuid, isize>, dependencies: &HashMap<Uuid, bool>) -> bool {
